...
export function AppShell({ children, title, subtitle }: AppShellProps) {
  ...
  useEffect(() => {
    ...
    if (storedCmsUser) {
      try {
        setCmsUser(JSON.parse(storedCmsUser));
      } catch {
  ...
  const {
    ...
    enabled: Boolean(token) && userType !== "CMS",
    retry: false,
  });
  const currentName = userType === "CMS" ? cmsUser?.fullName : member?.fullName;
  ...
  async function handleLogout() {
    ...
    try {
      if (userType === "CMS") {
        await api.post("/auth/cms/logout");
      } else {
      ...
      await navigate({
        to: userType === "CMS" ? "/cms-login" : "/login",
      });
    }
  }
          ...
          {nav
            .filter((item) => {
              if (item.to === "/cms-users") {
                return userType === "CMS" && cmsUser?.role === "ADMINISTRATOR";
              }
                ...
                <button className="flex items-center gap-2 rounded-md p-1 hover:bg-muted">
                  ...
                  <div className="hidden text-left sm:block">
                    <div className="text-sm font-medium leading-tight">
                      {isLoading ? "Loading..." : isError ? "Member" : currentName || "Member"}
                    </div>

                    <div className="text-[11px] text-muted-foreground">
                      {member
                        ? `${member.status} · ${member.identityVerificationStatus}`
                        : "Account"}
                    </div>
                  </div>
              ...
              <DropdownMenuContent align="end" className="w-56">
                <DropdownMenuLabel>
                  <div className="space-y-1">
                    ...
                    {(userType === "CMS" ? cmsUser?.email : member?.email) && (
                ...
                <DropdownMenuItem className="text-loss" onClick={handleLogout}>
