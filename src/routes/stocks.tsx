...
// synchronizationIntervalMs in market-data.service.ts —
// polling faster than the backend updates won't show new data sooner.
...
function StocksPage() {
  ...
  return (
    ...
    >
      ...
      {isLoading ? (
        <StatePanel text="Loading stocks..." />
      ) : isError ? (
        <StatePanel
          text="Unable to load stocks."
          error
          action={
            <Button
              variant="outline"
        ...
        <StatePanel text="No stocks match the selected filters." />
      ) : (
        <>
...
function getStoredCmsUser() {
  ...
  try {
    return JSON.parse(storedCmsUser);
  } catch {
